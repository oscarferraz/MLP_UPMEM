...
void Run_Test(T *m1, struct dpu_set_t set, struct dpu_set_t dpu, uint32_t nr_dpus, uint32_t each_dpu, char m1_name[], char binary_path[]){
	...
	T* m2 = (T*)malloc(NR_DPUS*SIZE*sizeof(T));
	...
	DPU_FOREACH(set,dpu,each_dpu){
		// printf("test=%d\n", each_dpu*(L1_SIZE/2));
		DPU_ASSERT(dpu_prepare_xfer(dpu, &m2[each_dpu*SIZE]));
	}
	//printf("for ended with line: %d\n", current_line);	
	DPU_ASSERT(dpu_push_xfer(set, DPU_XFER_FROM_DPU, m1_name, 0, sizeof(T) *  SIZE, DPU_XFER_DEFAULT));
	...
	for (int i = 0; i < SIZE; i++) {
		#if IS_FLOAT == 1
			fprintf(file3, "%f ", m2[i]);
		#endif
		#if IS_INT == 1
			fprintf(file3, "%d ", m2[i]);
		#endif
		fprintf(file3, "\n");
	}
