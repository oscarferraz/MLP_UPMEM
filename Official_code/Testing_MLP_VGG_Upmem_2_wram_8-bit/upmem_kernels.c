...
BARRIER_INIT(my_barrier, NR_TASKLETS); //syncronization
...
#if GEMM == 1
	void  kDot_m1_m2T_L1( __dma_aligned T const *m1, __dma_aligned T const *m2, __dma_aligned T *output){
		for(int i = 0; i < (TRAINING_SIZE/NR_DPUS); i++){
			for(int j = me()*L1_SIZE/NR_TASKLETS; j < me()*(L1_SIZE/NR_TASKLETS)+(L1_SIZE/NR_TASKLETS); j++){
				...
				#if IS_FLOAT == 1
					output[i*L1_SIZE+j]= (output[i*L1_SIZE+j] > 0.0) ? output[i*L1_SIZE+j] : 0.0;
				#endif
				#if IS_INT == 1
					output[i*L1_SIZE+j]= (output[i*L1_SIZE+j] > 0) ? output[i*L1_SIZE+j] : 0;
				#endif
	...
	void  kDot_m1_m2T_L2( __dma_aligned T const *m1, __dma_aligned T const *m2, __dma_aligned T *output){
		for(int i = 0; i < (TRAINING_SIZE/NR_DPUS); i++){
			for(int j = me()*L2_SIZE/NR_TASKLETS; j <  me()*(L2_SIZE/NR_TASKLETS)+(L2_SIZE/NR_TASKLETS); j++){
				...
				#if IS_FLOAT == 1
					output[i*L2_SIZE+j]= (output[i*L2_SIZE+j] > 0.0) ? output[i*L2_SIZE+j] : 0.0;
				#endif
				#if IS_INT == 1
					output[i*L2_SIZE+j]= (output[i*L2_SIZE+j] > 0) ? output[i*L2_SIZE+j] : 0;
				#endif
				/* if(me()==0){
					printf("output[%d]=%f\n", j,  output[j]);
				} */
		...
		void  kDot_m1_m2T_Output( __dma_aligned T const *m1, __dma_aligned T const *m2, __dma_aligned T *output){
			// for(int i = 0; i < (TRAINING_SIZE/NR_DPUS); i++){
	...
	#else
		void  kDot_m1_m2T_Output( __dma_aligned T const *m1, __dma_aligned T const *m2, __dma_aligned T *output, __dma_aligned T *sum){
	...
	void  kDot_m1_m2T_L1( __dma_aligned T const *m1, __dma_aligned T const *m2, __dma_aligned T *output){
		for(int j = me()*L1_SIZE/NR_TASKLETS; j < me()*(L1_SIZE/NR_TASKLETS)+(L1_SIZE/NR_TASKLETS); j++){
			...
			#if IS_FLOAT == 1
				output[j]= (output[j] > 0.0) ? output[j] : 0.0;
			#endif
			#if IS_INT == 1
	...
	void  kDot_m1_m2T_L2( __dma_aligned T const *m1, __dma_aligned T const *m2, __dma_aligned T *output){
		...
		void  kDot_m1_m2T_Output( __dma_aligned T const *m1, __dma_aligned T const *m2, __dma_aligned T *output, __dma_aligned T *sum){
			...
			if(me()==0){
				#if IS_FLOAT == 1
					output[0]= (sum[0] > 0.0) ? sum[0] : 0.0;
				#endif
				#if IS_INT == 1
					output[0]= (sum[0] > 0) ? sum[0] : 0;
				#endif
			}
			/* if(me()==0){
				printf("output[%d]=%f\n", me(),  output[me()]);
