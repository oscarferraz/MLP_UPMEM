...
void Run_KDot_ReLU_L1(T *m1, T *m2, T *m3, struct dpu_set_t set, struct dpu_set_t dpu, uint32_t nr_dpus, uint32_t each_dpu, char m1_name[], char m2_name[], char m3_name[], char binary_path[]){
...
void Run_KDot_ReLU_L2(T *m1, T *m2, T *m3, struct dpu_set_t set, struct dpu_set_t dpu, uint32_t nr_dpus, uint32_t each_dpu, char m1_name[], char m2_name[], char m3_name[], char binary_path[]){
...
void Run_KDot_ReLU_Output(T *m1, T *m2, T *m3, struct dpu_set_t set, struct dpu_set_t dpu, uint32_t nr_dpus, uint32_t each_dpu, char m1_name[], char m2_name[], char m3_name[], char binary_path[]){
	...
	DPU_FOREACH(set,dpu,each_dpu){
		#if MULTIPLE < 4
			DPU_ASSERT(dpu_prepare_xfer(dpu, &m3[each_dpu * 4 * OUTPUT_SIZE]));
		#elif MULTIPLE > 4
	...
	//printf("for ended with line: %d\n", current_line);	
	#if MULTIPLE < 4
		DPU_ASSERT(dpu_push_xfer(set, DPU_XFER_FROM_DPU, m3_name, 0, sizeof(T) * 4 * OUTPUT_SIZE, DPU_XFER_DEFAULT));
	#elif MULTIPLE > 4
...
int main(){
	...
	//PRED AND PRED_DELTA
	#if MULTIPLE < 4
		size_t y_size = NR_DPUS*4*sizeof(T);
	#elif MULTIPLE > 4
	...
	if(alloc_dpus(&set, &nr_dpus)==0){
  	...
  	Run_KDot_ReLU_L1(h_X, h_W0, h_layer_1, set, dpu, nr_dpus, each_dpu, "d_X", "d_W0", "d_l1", KDOT_RELU_BINARY);
	Run_KDot_ReLU_L2(h_layer_1, h_W1, h_layer_2, set, dpu, nr_dpus, each_dpu, "d_l1", "d_W1", "d_l2", KDOT_RELU2_BINARY);
	Run_KDot_ReLU_Output(h_layer_2, h_W2, h_pred, set, dpu, nr_dpus, each_dpu, "d_l2", "d_W2", "d_pred", KDOT_SIGMOID3_BINARY);
