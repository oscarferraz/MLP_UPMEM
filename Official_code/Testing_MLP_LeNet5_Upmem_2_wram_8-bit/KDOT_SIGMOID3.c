...
__host T d_l2[(TRAINING_SIZE/NR_DPUS)*L2_SIZE];
__host T d_W2[L2_SIZE*OUTPUT_SIZE];

#if MULTIPLE < 4
	__host T d_pred[NR_DPUS*OUTPUT_SIZE*4];
#elif MULTIPLE > 4
	__host T d_pred[NR_DPUS*OUTPUT_SIZE*8];
#else
	__host T d_pred[(TRAINING_SIZE/NR_DPUS)*OUTPUT_SIZE];
#endif
