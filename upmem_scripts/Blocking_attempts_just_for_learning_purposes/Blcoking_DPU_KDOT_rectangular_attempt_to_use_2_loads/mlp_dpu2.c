...
__mram float d_test1[test1_rows/NR_DPUS*test1_cols];
...
int main()
{
	...
	//*aux = kDot(NR_TASKLETS, d_test1, d_test2, d_res, 60, 50, 60, barr_addr);
	...
	for(int i = 0; i < test1_rows/NR_DPUS*test1_cols; i++)
	{
		printf("test1 nr: %d Result: %f\n",i ,d_test1[i]);
	}

	printf("\n");
	for(int i = 0; i < test2_rows/NR_DPUS*test2_cols; i++)
	{
