...
__mram_ptr T* kDot(const int n_threads, __mram_ptr T const *m1, __mram_ptr T const *m2, __mram_ptr T *output, __dma_aligned T cache1[CACHE_SIZE], __dma_aligned T cache2[CACHE_SIZE], const int m1_rows, const int m1_columns, const int m2_columns, struct barrier_t * barrier_address)
{	
	...
	*/
	...
	if(me() == 0)
	{
		for(int j = 0; j < m1_rows; j++)
		{
			...
			{
				printf("%.1f ", output[j*m2_columns+k]);
			}
			printf("\n");
		}
	}
