...
__mram float d_test1[m_size/NR_DPUS];
__mram float d_test2[m_size2/NR_DPUS];
__mram float d_res[m_size3/NR_DPUS];
...
//__dma_aligned float cache1[m_size/NR_DPUS]; // cache in WRAM to perform transfer
//__dma_aligned float cache2[m_size2/NR_DPUS]; // cache in WRAM to perform transfer
...
int main()
{
	...
	//test kernels
	aux = kMartixByMatrixElementwise(NR_TASKLETS, rows, cols, d_test1, d_test2, d_res, barr_addr);
