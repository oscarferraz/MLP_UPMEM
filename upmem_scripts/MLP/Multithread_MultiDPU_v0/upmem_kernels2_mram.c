...
__mram_ptr float* kMartixByMatrixElementwise(const int n_threads, const int width, const int height, __mram_ptr float const *m1, __mram_ptr float const *m2, __mram_ptr float *output, struct barrier_t * barrier_address)
{
...
__mram_ptr float* kMartixSubstractMatrix(const int n_threads, const int width, const int height, __mram_ptr float const *m1, __mram_ptr float const *m2, __mram_ptr float *output, struct barrier_t * barrier_address)
{
	int max_threads = width * height / 2;
	int n_elem = width * height / n_threads;
	if(n_threads > max_threads)
...
__mram_ptr float* kSigmoid(const int n_threads, const int width, const int height, __mram_ptr float const *m1, __mram_ptr float *output, struct barrier_t * barrier_address)
{
...
__mram_ptr float* kSigmoid_inference(const int n_threads, const int width, const int height, __mram_ptr float const *m1, __mram_ptr float *output, struct barrier_t * barrier_address)
{
...
__mram_ptr float* kSigmoid_d(const int n_threads, const int width, const int height, __mram_ptr float const *m1, __mram_ptr float *output, struct barrier_t * barrier_address)
{
	int max_threads = width * height / 2;
	int n_elem = width * height / n_threads;
	if(n_threads > max_threads)
...
__mram_ptr float* kDot_m1T_m2(const float lr, const int n_threads, __mram_ptr float const *m1, __mram_ptr float const *m2, __mram_ptr float *output, const int m1_rows, const int m1_columns, const int m2_columns, struct barrier_t * barrier_address)
{
		...
		const float lr)
{
	for (unsigned i = 0; i < 500; ++i)
	{
        ...
        kSigmoid(n_threads, l1_w, X_h, kDot(n_threads, X, W0, l1, X_h, X_w, l1_w, barrier_address), l1, barrier_address);
        //printf("Second Sigmoid: \n");
        kSigmoid(n_threads, y_w, X_h, kDot(n_threads, l1, W1, pred, X_h, l1_w, y_w, barrier_address), pred, barrier_address);
        //backpropagation: gradient of y - x
		//printf("Backprop: \n");
		kMartixByMatrixElementwise(n_threads, y_w, X_h, kMartixSubstractMatrix(n_threads,y_w, X_h, y, pred, pred_d, barrier_address), kSigmoid_d(n_threads, y_w, X_h, pred, buffer, barrier_address), pred_d, barrier_address);
      	//printf("\n");
      	kMartixByMatrixElementwise(n_threads, l1_w, X_h, kDot_m1_m2T(n_threads, pred_d, W1, l_1_d, X_h, y_w, l1_w, barrier_address), kSigmoid_d(n_threads, l1_w, X_h, l1, buffer, barrier_address), l_1_d, barrier_address);
        //update weights
        //printf("Update weights W1\n");
        kDot_m1T_m2(lr, n_threads, l1, pred_d, W1, X_h, l1_w, y_w, barrier_address);
        //printf("Update weights W0\n");
        kDot_m1T_m2(lr, n_threads, X, l_1_d, W0, X_h, X_w, l1_w, barrier_address);
    	//printf("iter: %d\n", i);
	}
		...
		__mram_ptr float *W1)
{
	...
	printf("\nKtest began");
    kSigmoid_inference(n_threads, l1_w, X_h, kDot_inference(n_threads, X, W0, l1, X_h, X_w, l1_w, barrier_address), l1, barrier_address);
    kSigmoid_inference(n_threads, y_w, X_h, kDot_inference(n_threads, l1, W1, pred, X_h, l1_w, y_w, barrier_address), pred, barrier_address);
