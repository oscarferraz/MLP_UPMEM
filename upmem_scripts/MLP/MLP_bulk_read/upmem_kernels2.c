...
__mram_ptr float* kSigmoid(const int n_threads, const int width, const int height, __mram_ptr float const *m1, __mram_ptr float *output, __dma_aligned float cache1[CACHE_SIZE], struct barrier_t * barrier_address)
{
  ...
  if(me() < max_threads)
	...
	for(int i = me()*n_elem; i < (me()+1)*n_elem; i+=1)
	{
		...
		output[i] = 1.0 / (1.0 + exponential(-m1[i]));
...
__mram_ptr float* kDot_m1T_m2(const int n_threads, __mram_ptr float const *m1, __mram_ptr float const *m2, __mram_ptr float *output, __dma_aligned float cache1[CACHE_SIZE], __dma_aligned float cache2[CACHE_SIZE], const int m1_rows, const int m1_columns, const int m2_columns, struct barrier_t * barrier_address)
{
		...
		__mram_ptr float *buffer)
{
	for (unsigned i = 0; i < 50; ++i)
	{
        ...
        kDot_m1T_m2(n_threads, l1, pred_d, W1, cache1, cache2, X_h, l1_w, y_w, barrier_address);
        printf("Update weights W0\n");
        kDot_m1T_m2(n_threads, X, l_1_d, W0, cache1, cache2, X_h, X_w, l1_w, barrier_address);
    	//printf("iter: %d\n", i);
	}
	//barrier_wait(barrier_address);
    	if(me() == 0)
