...
int main()
{
	...
	const int m_size2 = 60*40;
	const int m_size3 = 50*40;
    ...
    DPU_ASSERT(dpu_copy_to(set, "d_test1", 0, test1, sizeof(float) * m_size));
    DPU_ASSERT(dpu_copy_to(set, "d_test2", 0, test2, sizeof(float) * m_size2));
    DPU_ASSERT(dpu_copy_to(set, "d_res", 0, res, sizeof(float) * m_size3));
