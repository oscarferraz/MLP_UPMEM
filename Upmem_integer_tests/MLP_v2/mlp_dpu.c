...
__mram int d_buffer[NR_ELEM_PER_DPU];
__mram int d_layer_1_delta[NR_ELEM_PER_DPU];
__mram int d_W1[NR_ELEM_PER_DPU];
__mram int d_Y[NR_ELEM_PER_DPU];
__mram int d_pred[NR_ELEM_PER_DPU];
__mram int d_pred_delta[NR_ELEM_PER_DPU];
...
__dma_aligned int cachea[CACHE_SIZE]; // cache in WRAM to perform transfer
__dma_aligned int cacheb[CACHE_SIZE]; // cache in WRAM to perform transfer

//For KTest function
__mram int d_test[4*28];
__mram int d_test_pred[28];
__mram int d_test_l1[28*8];



//__host float X_res[NR_ELEM_PER_DPU * 1];
__mram int X_res[NR_ELEM_PER_DPU];
...
int main()
{
	...
	/*for(int i = 0; i < TRAINING_SIZE*TRAINING_DIM; i++)
	{
		...
		d_pred, d_pred_delta, d_W0, d_W1, d_buffer,1);
